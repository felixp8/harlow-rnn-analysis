...
# save_path = "/home/fpei2/learning/harlow_analysis/runs/harlowdelay3_gru256/trajectories/epoch19999-v1_sample_multi{}.npz"
...
# sample_multi{n}.npz: env seed = 0, random seed = 0
...
for n in range(num_objsets):
    ...
    for (r, o1, o2) in conditions:
        ...
        while any([(len(t) < trials_per_condition) for t in trajectories]):
            ...
            if skip:
                ...
                thresh = 300 if not any([(len(t) == trials_per_condition) for t in trajectories]) else 100
                if num_skipped >= thresh:
        ...
        for i in range(len(trajectories)):
            ...
            all_data.append((obs, actions, rewards, states, np.array([n, r, o1, i, o2]), num_skipped))

    obs, actions, rewards, states, condition, num_skipped = zip(*all_data)
    obs = np.stack(obs)
    actions = np.stack(actions)
    rewards = np.stack(rewards)
    states = np.stack(states)
    ...
    print(f'Covered {obs.shape[0]} / 16 conditions')

    import pdb; pdb.set_trace()

    np.savez(save_path.format(n), obs=obs, actions=actions, rewards=rewards, states=states, condition=condition, obj1=obj1, obj2=obj2, num_skipped=num_skipped)
